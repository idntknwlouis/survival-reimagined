...
public class MillstoneJEIRecipe implements Recipe<RecipeInput> {
	private final List<ItemStack> output;
	private final NonNullList<Ingredient> recipeItems;
	private final List<Integer> integers;

	public MillstoneJEIRecipe(List<ItemStack> output, NonNullList<Ingredient> recipeItems, List<Integer> integers) {
		this.output = output;
	...
	public static final class Type implements RecipeType<MillstoneJEIRecipe> {
	...
	public static final class Serializer implements RecipeSerializer<MillstoneJEIRecipe> {
		public static final Serializer INSTANCE = new Serializer();

		private static final MapCodec<MillstoneJEIRecipe> CODEC = RecordCodecBuilder.mapCodec(builder -> builder.group(
		...
		).apply(builder, MillstoneJEIRecipe::new));

		private static final StreamCodec<RegistryFriendlyByteBuf, MillstoneJEIRecipe> STREAM_CODEC =
				StreamCodec.of(Serializer::toNetwork, Serializer::fromNetwork);

		@Override
		public MapCodec<MillstoneJEIRecipe> codec() {
			return CODEC;
		}

		@Override
		public StreamCodec<RegistryFriendlyByteBuf, MillstoneJEIRecipe> streamCodec() {
			return STREAM_CODEC;
		}

		private static MillstoneJEIRecipe fromNetwork(RegistryFriendlyByteBuf buf) {
			...
			numbers.replaceAll(ignored -> buf.readVarInt());
			return new MillstoneJEIRecipe(outputs, inputs, numbers);
		}

		private static void toNetwork(RegistryFriendlyByteBuf buf, MillstoneJEIRecipe recipe) {
			...
			for (Integer value : recipe.integers()) {
